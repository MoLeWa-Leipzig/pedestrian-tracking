...
class TrackingViewer:
    ...
    def generate_view(self, objects, image_left_ocv, img_scale, current_camera_pose, tracking_view, tracking_enabled):
        ...
        if tracking_enabled:
            ...
            #self.drawTracks(tracking_view, objects.object_list)
        else:
            self.drawPosition(objects, tracking_view, current_camera_pose)

#    def drawTracks(self, tracking_view, objs):
#        for obj in objs:
#            pos = obj.position
#            x_world = pos[0]
#            z_world = pos[2]

#            cv2.circle(tracking_view, (int(pos[0]), int(cv_start_point[1])), 6, clr, -1)
    ...
    def render_2D(self, left_display, img_scale, objects, render_mask, isTrackingON):
        ...
        for obj in objects:
            if renderObject(obj, isTrackingON):
                ...
                cv2.line(left_display, tuple(top_left_corner), tuple(top_right_corner), base_color, line_thickness)
                cv2.line(left_display, tuple(bottom_left_corner), tuple(bottom_right_corner), base_color, line_thickness)
                ...
                cv2.putText(left_display, str(obj.label), (int(position_image[0]) - 20, int(position_image[1]) - 20),
                            cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (255, 255, 255, 255), 1)
                cv2.putText(left_display, "ID " + str(obj.id), (int(position_image[0]) - 20, int(position_image[1]) - 50),
                            cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (255, 255, 255, 255), 1)
                
                #if math.isfinite(obj.position[2]):
                #    text = "X: {:.1f} Y: {:.1f} Z: {:.1f}M".format(obj.position[0], obj.position[1], obj.position[2])
                #    cv2.putText(left_display, text, (int(position_image[0]) - 20, int(position_image[1]) - 80),
                #                cv2.FONT_HERSHEY_COMPLEX_SMALL, 1.5, (255, 255, 255, 255), 1)

                #height = 0 #abs(obj.bounding_box[0][1] - obj.bounding_box[4][1])
                #text = f"Height: {height:.1f}"
                #cv2.putText(left_display, text, (int(position_image[0]) - 20, int(position_image[1]) - 110),cv2.FONT_HERSHEY_COMPLEX_SMALL, 1.5, (255, 255, 255, 255), 1)
...
def drawVerticalLine(display,start_pt,end_pt,clr,thickness):
    ...
    cv2.line(display,tuple(start_pt),tuple(pt1),clr,thickness)
    cv2.line(display,tuple(pt4),tuple(end_pt),clr,thickness)
